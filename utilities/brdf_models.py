...
def compute_mapp(sun_eci, obs_eci, q_IB, brdf_function, surfaces):
    '''
    ...
    # Break out inputs
    C_sunvis = inputs['C_sunvis']
    
    # Compute unit vectors
    sun_eci_hat = sun_eci/np.linalg.norm(sun_eci)
    obs_eci_hat = obs_eci/np.linalg.norm(obs_eci)
    
    if brdf_function == lambertian_sphere:
        
        # Compute BRDF for the sphere
        brdf_params = surfaces[0]['brdf_params']
        brdf = brdf_function(sun_eci_hat, obs_eci_hat, brdf_params)
        
        # Compute Fobs per Reference 3
        sat_radius = brdf_params['sat_radius']
        sat_rg = np.linalg.norm(obs_eci)     
        phase_angle = acos(float(np.dot(sun_eci_hat.T, obs_eci_hat)))
        
        sum_Fobs = Csun_vis * (4./9.) * brdf * (sat_radius/sat_rg)**2. * \
        ...
        for ii in surfaces:
            ...
            Fobs = Fsun*A*dot_n_obs/float(np.dot(obs_eci.T, obs_eci))
...
def lambertian_sphere(sun_eci_hat, obs_eci_hat, brdf_params):
    '''
...
def ashikhmin_shirley(sun_eci_hat, obs_eci_hat, brdf_params):
    '''
...
def ashikhmin_premoze(sun_eci_hat, obs_eci_hat, brdf_params):
    '''
