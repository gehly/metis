function [Xref_mat,Xk_mat,P_mat,Pck_mat,Pc_mat,resids] = ls_ckf_cc_analysis(obs_data,intfcn,H_fcn,inputs)
%This script implements a Least Squares Conventional Kalman Filter.  It
%contains computations for Consider Covariance Analysis (does not use the
...
%Map final xhat and Pck back to start epoch
Pcf = [Pck  Pxck;Pxck'  Pcc_bar];
xf_hat = xhat;

%Compute mapping matrices phi and theta
int0 = [Xref;phi0_v;theta0_v];
tin = [t_obs(end) t_obs(1)];
[tout,intout] = ode45(intfcn,tin,int0,ode45_options,inputs);
xout = intout(end,:);
phi_v = xout(n+1:n+n^2)';
phi = reshape(phi_v,n,n);   
...
xo_hat = phi*xf_hat;
psi = [phi theta;zeros(1,n) 1];
Pco = psi*Pcf*psi';


%Compute new reference trajectory and propagate Pco through all time steps
Xref = Xo_ref + xo_hat;
Xref_mat(:,1) = Xref;
Pc_mat(:,:,1) = Pco;
